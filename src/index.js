...
function generatePoem(event) {
  ...
  const context = "You are a poem expert and you love to write short poems.  Your mission is to generate a four line poem in basic HTML format and separate each line with a <br />.  Make sure to follow the user instructions.  Sign the poem with the poet's name inside a <strong> element after the last line of the poem like this: <strong>Poet's name</strong>.";
  const apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  const poem = document.querySelector("#poem");
  poem.innerHTML = `<div class="generating">Generating a poem about ${poemTopic.value} by ${poet.value}</div>`;
